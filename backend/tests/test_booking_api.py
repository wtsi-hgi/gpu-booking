"""Tests for booking endpoint validation and warning behaviour."""
...
from db.models import Admin, Base, Booking, BookingStatus, GpuHostType, WorkflowType
...
async def _get_host_type_id_by_gpu_type(gpu_type: str) -> int:
    """Return a GPU host type ID by GPU type."""

    async with async_session_factory() as session:
        result = await session.execute(
            select(GpuHostType).where(GpuHostType.gpu_type == gpu_type)
        )
        host_type = result.scalar_one()
        return host_type.id
...
@pytest.mark.anyio
async def test_list_bookings_filters_by_gpu_host_type_id() -> None:
    h100_id, workflow_type_id = await _get_reference_ids()
    h200_id = await _get_host_type_id_by_gpu_type("H200")

    await _insert_booking(
        user_email="h100@example.com",
        gpu_host_type_id=h100_id,
        workflow_type_id=workflow_type_id,
        start_date=date(2026, 3, 2),
        end_date=date(2026, 3, 4),
    )
    await _insert_booking(
        user_email="h200@example.com",
        gpu_host_type_id=h200_id,
        workflow_type_id=workflow_type_id,
        start_date=date(2026, 3, 2),
        end_date=date(2026, 3, 4),
    )
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get(
            "/api/v1/bookings",
            params={"gpu_host_type_id": h100_id},
        )

    assert response.status_code == 200
    body = response.json()
    assert len(body) == 1
    assert body[0]["gpu_host_type_id"] == h100_id
...
@pytest.mark.anyio
async def test_create_booking_returns_201_with_unconfirmed_status_and_host_shape() -> (
    None
):
    host_type_id, workflow_type_id = await _get_reference_ids()
    ...
    assert body["gpu_host_type_id"] == host_type_id
...
@pytest.mark.anyio
async def test_create_booking_rejects_unknown_gpu_host_type() -> None:
    _, workflow_type_id = await _get_reference_ids()
    payload = _booking_payload(
        gpu_host_type_id=999999,
        workflow_type_id=workflow_type_id,
    ...
    assert response.json() == {"detail": "GPU host type not found"}
...
async def test_create_booking_returns_40_percent_warning_when_user_exceeds_limit() -> (
    ...
    async with async_session_factory() as session:
        host_type = await session.get(GpuHostType, host_type_id)
        assert host_type is not None
        host_type.total_count = 5

        other_host_types = await session.execute(
            select(GpuHostType).where(GpuHostType.id != host_type_id)
        )
        for other_host_type in other_host_types.scalars().all():
            other_host_type.total_count = 0
...
@pytest.mark.anyio
@pytest.mark.parametrize("host_count", [0, -1])
async def test_create_booking_returns_422_for_non_positive_host_count(
    host_count: int,
) -> None:
    host_type_id, workflow_type_id = await _get_reference_ids()
    payload = _booking_payload(
