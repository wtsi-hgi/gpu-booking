...
export function getFrontendOidcConfig(): FrontendOidcConfig | null {
  const issuerUrl = readEnv('OIDC_ISSUER_URL', 'OIDC_ISSUER', 'OKTA_ISSUER')
  const clientId = readEnv('OIDC_CLIENT_ID', 'OKTA_CLIENT_ID')
  const clientSecret = readEnv('OIDC_CLIENT_SECRET', 'OKTA_CLIENT_SECRET')

  if (!issuerUrl || !clientId || !clientSecret) {
    return null
  }

  const configuredScopes = readEnv('OIDC_SCOPES')
